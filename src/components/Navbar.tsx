...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // NEW: Fetch categories from the database when the Navbar loads
    ...
    <nav className="bg-white border-b border-stone-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-8 flex justify-between items-center h-24"> 
        ...
        <div className="flex items-center space-x-8 font-medium text-stone-700">
              ...
              <div className="flex flex-col py-2">
                ...
                {categories.length > 0 ? (
                  categories.map((category) => (
                    <Link key={category.slug} href={`/shop?category=${category.slug}`} className="px-4 py-2 hover:bg-stone-50 hover:text-amber-700 transition-colors text-sm">
                      {category.name}
                    </Link>
                  ))
                ) : (
                  <span className="px-4 py-2 text-sm text-stone-400 italic">Loading categories...</span>
                )}
          ...
          <Link href="/favourites" className="flex items-center justify-center w-10 h-10 hover:text-rose-500 transition-colors bg-white rounded-full border border-stone-200 shadow-sm relative group" title="Favourites">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>
            {favouriteCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center shadow-sm relative overflow-hidden">
                <span className="relative z-10">{favouriteCount}</span>
                <span className="absolute inset-0 bg-white/20 blur-[1px]"></span>
              </span>
            )}
          </Link>
